	...
	async do() {
		...
		if (layer.type == 'image') {
			...
			if (config.layers.length == 1 && (config.layer.width == 0 || config.layer.width === null)
					&& (config.layer.height == 0 || config.layer.height === null) && config.layer.data == null && !config.is_custom_document) {
				// Remove first empty layer if not a custom document
			...
			if (layer.link == null) {
				if (typeof layer.data == 'object') {
					...
					layer.data = null;
					if (config.is_custom_document) {
						if (this.settings.x != null && this.settings.y != null) {
							layer.x = this.settings.x;
							layer.y = this.settings.y;
						} else {
							layer.x = Math.max(0, Math.round((config.WIDTH - layer.width) / 2));
							layer.y = Math.max(0, Math.round((config.HEIGHT - layer.height) / 2));
						}
						autoresize_as = null;
					} else {
