...
class File_open_class {
			...
			FR.onload = function (event) {
				if (this.file.type.match('image.*')) {
					...
					if (app.GUI && app.GUI.GUI_tabs) {
						app.GUI.GUI_tabs.set_active_tab_name(this.file.name);
					}
				}
				else {
					//json
	...
	/**
	 * check if url has url params, for example: https://viliusle.github.io/PhotoClone/?image=http://i.imgur.com/ATda8Ae.jpg
	 */
	maybe_file_open_url_handler() {
		...
		if (url_params.image != undefined) {
			...
			// WebMCP Automation: Handle 'action' param
			if (url_params.action != undefined) {
				_this.execute_webmcp_action(url_params.action);
			}
		}
	}

	execute_webmcp_action(action) {
		var _this = this;
		// Wait for modules to be loaded and image to be ready
		let attempts = 0;
		const interval = setInterval(() => {
			attempts++;
			const aiModule = app.GUI && app.GUI.modules && app.GUI.modules['ai/tools'];
			const layers = app.Layers ? app.Layers.get_layers() : [];
			
			if (aiModule && layers.length > 0) {
				clearInterval(interval);
				console.log('WebMCP: Executing action', action);
				
				switch (action) {
					case 'ai_remove_bg':
						aiModule.remover_fundo_ai();
						break;
					case 'ai_colorize':
						aiModule.colorizar_ai();
						break;
					case 'ai_upscale':
						aiModule.upscale_ai();
						break;
			...
			if (attempts > 50) { // Timeout 5s
				clearInterval(interval);
				console.warn('WebMCP: Timeout waiting for AI module');
			}
		}, 100);
	}
	...
	search_hub(){
		this.GUI_tools.activate_tool('shape');
	}
	...
	search_illustrations(){
		this.GUI_tools.activate_tool('illustrations');
	}

	search_icons(){
		this.GUI_tools.activate_tool('icons');
	}

	search_wallpapers(){
		this.GUI_tools.activate_tool('wallpapers');
	}

	search_stickers(){
		this.GUI_tools.activate_tool('stickers');
	}

	search_emojis(){
		this.GUI_tools.activate_tool('emojis');
	}
	...
	import_image() {
		...
		document.getElementById("tmp").innerHTML = '';
		var a = document.createElement('input');
		a.setAttribute("id", "file_import");
		a.type = 'file';
		document.getElementById("tmp").appendChild(a);
		document.getElementById('file_import').addEventListener('change', function (e) {
			_this.import_handler(e);
		}, false);

		//force click
		document.querySelector('#file_import').click();
	}

	async import_handler(e) {
		var _this = this;
		var files = e.target.files;
		...
		if (file.name.match('.json')) {
			var FR = new FileReader();
			FR.onload = function (event) {
				_this.load_json(event.target.result);
			};
			FR.readAsText(file);
			return;
		}

		this.SAVE_NAME = file.name.split('.')[file.name.split('.').length - 2];
		...
		FR.onload = function (event) {
			...
			img.onload = async function () {
				...
				// Reset zoom
				await _this.Base_gui.GUI_preview.zoom(100);
				...
				await app.State.do_action(
					new app.Actions.Bundle_action('import_image', 'Import Image', actions)
				);

				if (_this.Base_gui && _this.Base_gui.GUI_preview && typeof _this.Base_gui.GUI_preview.zoom_auto == 'function') {
					_this.Base_gui.GUI_preview.zoom_auto(true);
				}
				config.need_render = true;
			};
