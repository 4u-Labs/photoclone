...
class Base_state_class {
	...
	async jump_to_history(targetIndex) {
		if (typeof targetIndex !== 'number' || isNaN(targetIndex)) return;
		if (targetIndex < 0) targetIndex = 0;
		if (targetIndex > this.action_history.length) targetIndex = this.action_history.length;

		if (targetIndex === this.action_history_index) return;

		while (this.action_history_index > targetIndex && this.can_undo()) {
			this.action_history_index--;
			await this.action_history[this.action_history_index].undo();
		}
		while (this.action_history_index < targetIndex && this.can_redo()) {
			const action = this.action_history[this.action_history_index];
			await action.do();
			this.action_history_index++;
		}
	...
	on_history_change() {
		if (app.GUI && app.GUI.GUI_layers && typeof app.GUI.GUI_layers.render_history === 'function') {
			app.GUI.GUI_layers.render_history();
		}
	}
