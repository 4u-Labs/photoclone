...
class Base_layers_class {
	...
	pre_render() {
		...
		this.ctx.clearRect(
			0,
			0,
			Math.max(this.canvas.width, config.WIDTH * config.ZOOM),
			Math.max(this.canvas.height, config.HEIGHT * config.ZOOM)
		);
	}
	...
	render_preview(layers) {
		const previewEl = document.getElementById("canvas_preview");
		if (!previewEl) return;

		// Calculate proportional preview size maintaining exact document aspect ratio
		const max_w = 176;
		const max_h = 130;
		const aspect_ratio = (config.WIDTH && config.HEIGHT) ? (config.WIDTH / config.HEIGHT) : (16 / 9);
		let w, h;

		if (aspect_ratio >= max_w / max_h) {
			w = max_w;
			h = Math.max(30, Math.round(max_w / aspect_ratio));
		} else {
			h = max_h;
			w = Math.max(30, Math.round(max_h * aspect_ratio));
		}

		if (this.Base_gui && this.Base_gui.GUI_preview) {
			this.Base_gui.GUI_preview.PREVIEW_SIZE = {w: w, h: h};
		}
		...
		if (previewEl.width !== w || previewEl.height !== h) {
			...
			const innerEl = document.getElementById("canvas_preview_inner");
			if (innerEl) {
			...
			if (this.Base_gui && this.Base_gui.GUI_preview) {
				this.Base_gui.GUI_preview.prepare_canvas();
			}
		}
		...
		const scale_factor_x = w / config.WIDTH;
		const scale_factor_y = h / config.HEIGHT;
		newCanvas.getContext("2d").scale(scale_factor_x, scale_factor_y);
		this.render_objects(this.ctx_preview, newCanvas, layers, () => {
			this.ctx_preview.save();
			//prepare scale
			this.ctx_preview.scale(scale_factor_x, scale_factor_y);
		});
	...
	render_object(ctx, object, is_preview) {
		...
		if (object.type == "image") {
			...
			var srcImg = object.link_canvas != null ? object.link_canvas : object.link;
			if (srcImg) {
				var canDraw = true;
				if (srcImg instanceof HTMLImageElement) {
					canDraw = srcImg.complete && srcImg.naturalWidth > 0 && srcImg.naturalHeight > 0;
				} else if (srcImg instanceof HTMLCanvasElement || (typeof OffscreenCanvas !== 'undefined' && srcImg instanceof OffscreenCanvas)) {
					canDraw = srcImg.width > 0 && srcImg.height > 0;
				}
				if (canDraw) {
					try {
						ctx.drawImage(
