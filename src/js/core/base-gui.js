...
import GUI_tabs_class from './gui/gui-tabs.js';
...
class Base_gui_class {
	...
	render_main_gui() {
		...
		// Determine language early so components render with correct active state
		var savedLang = localStorage.getItem('photoclone_lang') || this.Helper.getCookie('language');
		var params = this.Helper.get_url_parameters();
		if (params.lang != undefined) {
			savedLang = params.lang.replace(/([^a-z]+)/gi, '');
		}
		if (!savedLang) {
			savedLang = 'pt';
		}
		config.LANG = savedLang;
		this.Helper.setCookie('language', config.LANG);
		localStorage.setItem('photoclone_lang', config.LANG);
		...
		// Initialize the unified credits and billing ecosystem
		if (this.modules['ai/tools'] && typeof this.modules['ai/tools'].init_credits_system === 'function') {
			this.modules['ai/tools'].init_credits_system();
		}
		...
		// Translate entire DOM after all UI components have been rendered
		if (config.LANG !== 'en') {
			this.Tools_translate.translate(config.LANG, document.body);
		}
	}
	...
	set_events() {
		...
		this.GUI_menu.on('select_href', (href) => {
			if (href) {
	...
	load_translations() {
		var lang = this.Helper.getCookie('language') || localStorage.getItem('photoclone_lang');
		...
		// Default to 'pt' if nothing else is set
		if (lang == null) {
			lang = 'pt';
		}
	...
	render_canvas_background(canvas_id, gap) {
		...
		if (canvas_id == 'canvas_minipaint') {
			const scale = zoomView.getScale();
			const pos = zoomView.getPosition();
			const size = (gap * 2) * scale;
			
			// Use the original PNG grid but scale it
			target.style.backgroundSize = size + 'px ' + size + 'px';
			target.style.backgroundPosition = pos.x + 'px ' + pos.y + 'px';
		}
		else {
			target.style.backgroundSize = (gap * 2) + 'px ' + (gap * 2) + 'px';
