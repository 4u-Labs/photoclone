...
class Base_selection_class {
	...
	draw_selection() {
		...
		if (this.is_drag && this.mouse_lock == 'selected_object_actions' && !isRotated) {
			const snapTol = 6;
			const objCenterX = data.x + data.width / 2;
			const objCenterY = data.y + data.height / 2;
			...
			this.ctx.setLineDash([4 / config.ZOOM, 4 / config.ZOOM]);

			// Center X
			if (Math.abs(objCenterX - canvasCenterX) <= snapTol) {
				this.ctx.beginPath();
				this.ctx.moveTo(canvasCenterX, 0);
				this.ctx.lineTo(canvasCenterX, config.HEIGHT);
				this.ctx.stroke();
			}

			// Center Y
			if (Math.abs(objCenterY - canvasCenterY) <= snapTol) {
				this.ctx.beginPath();
				this.ctx.moveTo(0, canvasCenterY);
				this.ctx.lineTo(config.WIDTH, canvasCenterY);
				this.ctx.stroke();
			}
