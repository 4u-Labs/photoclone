...
var template = `
	<div id="text_detail_params">
		<div class="text-effects-header">
			<span>✨</span> <span class="trn">Quick Styles</span>
		</div>
		<div class="text-effects-grid">
			<button type="button" class="text-quick-style-btn" id="btn_text_shadow" title="Shadow">
				<span>🕶️</span> <span class="trn">Shadow</span>
			</button>
			<button type="button" class="text-quick-style-btn" id="btn_text_stroke" title="Stroke">
				<span>🎨</span> <span class="trn">Stroke</span>
			</button>
			<button type="button" class="text-quick-style-btn" id="btn_text_glow" title="Neon">
				<span>✨</span> <span class="trn">Neon</span>
			</button>
			<button type="button" class="text-quick-style-btn" id="btn_text_box" title="Highlight Box">
				<span>🏷️</span> <span class="trn">Highlight Box</span>
			</button>
		</div>
		<button type="button" class="btn-edit-text" id="detail_param_text">✏️ <span class="trn">Edit Text...</span></button>
		<div class="row">
...
class GUI_details_class {
	...
	render_text(events) {
		...
		const btnShadow = document.getElementById('btn_text_shadow');
		const btnStroke = document.getElementById('btn_text_stroke');
		const btnGlow = document.getElementById('btn_text_glow');
		const btnBox = document.getElementById('btn_text_box');

		if (btnShadow && btnStroke && btnGlow && btnBox && layer && layer.type === 'text' && layer.params) {
			btnShadow.classList.toggle('active', !!layer.params.shadow);
			btnStroke.classList.toggle('active', !!layer.params.stroke);
			btnGlow.classList.toggle('active', !!layer.params.glow);
			btnBox.classList.toggle('active', !!layer.params.bg_box);
		}
		...
		if (events) {
			...
			// Drop Shadow Button
			btnShadow.addEventListener('click', function (e) {
				...
				_this.Text.hasValueChanged = true;
				config.need_render = true;
				config.need_render_changed_params = true;
				_this.Base_layers.render();
				_this.render_text(false);
			});

			// Stroke Button
			btnStroke.addEventListener('click', function (e) {
				if (!config.layer || config.layer.type !== 'text') return;
			...
			// Outer Glow Button
			btnGlow.addEventListener('click', function (e) {
				if (!config.layer || config.layer.type !== 'text') return;
