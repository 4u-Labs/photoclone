...
class Helper_class {
	 ...
	 * @param end_y
	 */
	draw_special_line(ctx, start_x, start_y, end_x, end_y, color = '#ff007f'){
		const lineWidth = Math.max(1, 1.5 / (config.ZOOM || 1));
		...
		ctx.beginPath();
		ctx.moveTo(Math.round(start_x) + 0.5, Math.round(start_y) + 0.5);
		ctx.lineTo(Math.round(end_x) + 0.5, Math.round(end_y) + 0.5);
		ctx.stroke();
		ctx.restore();
	}
