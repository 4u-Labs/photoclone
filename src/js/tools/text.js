...
class Text_editor_class {
	...
	render(ctx, layer) {
		...
		try {
			...
			// Render Background Box (Tarja / Caixa de Destaque)
			if (layer.params && layer.params.bg_box) {
				const boxPadding = layer.params.bg_box_padding || 10;
				const boxColor = layer.params.bg_box_color || '#000000';
				const boxRadius = layer.params.bg_box_radius != null ? layer.params.bg_box_radius : 8;
				const boxOpacity = layer.params.bg_box_opacity != null ? layer.params.bg_box_opacity : 0.85;

				ctx.save();
				ctx.globalAlpha = (ctx.globalAlpha || 1) * boxOpacity;
				ctx.fillStyle = boxColor;
				
				const bX = Math.round(drawOffsetLeft - boxPadding);
				const bY = Math.round(drawOffsetTop - boxPadding);
				const bW = Math.round((boundary === 'dynamic' ? this.textBoundaryWidth : layer.width) + (boxPadding * 2));
				const bH = Math.round((boundary === 'dynamic' ? this.textBoundaryHeight : layer.height) + (boxPadding * 2));
			...
			// Configure Drop Shadow or Outer Glow
			if (layer.params && layer.params.shadow) {
				ctx.shadowColor = layer.params.shadow_color || 'rgba(0, 0, 0, 0.85)';
				ctx.shadowBlur = layer.params.shadow_blur || 10;
				ctx.shadowOffsetX = layer.params.shadow_x != null ? layer.params.shadow_x : 4;
				ctx.shadowOffsetY = layer.params.shadow_y != null ? layer.params.shadow_y : 4;
			} else if (layer.params && layer.params.glow) {
				ctx.shadowColor = layer.params.glow_color || '#00e5ff';
				ctx.shadowBlur = layer.params.glow_blur || 18;
				ctx.shadowOffsetX = 0;
				ctx.shadowOffsetY = 0;
			} else {
			...
			for (let line of this.lineRenderInfo.lines) {
				...
				for (let [localWrapIndex, wrap] of line.wraps.entries()) {
					...
					for (let [spanIndex, span] of wrap.spans.entries()) {
						...
						let fillStyle;
						if (fill_color.startsWith('#') || fill_color.startsWith('rgb')) {
							fillStyle = fill_color;
						}
						let stroke_size = ((span.meta.stroke_size != null) ? span.meta.stroke_size : metaDefaults.stroke_size);
						let strokeStyle;
						if (layer.params && layer.params.stroke && !stroke_size) {
							stroke_size = layer.params.stroke_size || 4;
						}
						if (stroke_size) {
							const stroke_color = (layer.params && layer.params.stroke && layer.params.stroke_color) ? layer.params.stroke_color : (span.meta.stroke_color || metaDefaults.stroke_color);
							if (stroke_color.startsWith('#') || stroke_color.startsWith('rgb')) {
