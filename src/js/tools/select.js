...
class Select_tool_class extends Base_tools_class {
	...
	async mousedown(e) {
		...
		if (this.Base_selection.mouse_lock != null) {
			...
			// Alt + Drag: Duplicate layer and drag the clone
			if (e.altKey && config.layer) {
				const Layer_duplicate = new Layer_duplicate_class();
				await Layer_duplicate.duplicate(0);
			}
	...
	render_overlay(ctx){
		...
		if (this.moving && (this.snap_line_info.x !== null || this.snap_line_info.y !== null)) {
